---
title: "Olympic Gymnastics Exploratory Data Analysis"
author: "Will Gertsch"
date: "July 31, 2018"
output:
...
Gymnastics is a sport that I believe has a lot of potential for analytic. There is a decent amount of data and domain knowledge of the sport is challenging but rewarding to work with. My goal for this analysis is to explore how gymnasts have changed over the years and how teams and individuals have performed.
...
Now we have the data for just gymnastics. I want to additionally prune this data to get rid of some columns we don't really need. I also changed the units used in Height and Weight from metric to imperial.
```{r}
...
Sure enough, those are some massive teams(looking at you GB). This explains why 1908 had so many competitors. The the same is the case with the 1952 games, but instead of there being a few, super-massive teams, there are a bunch of teams with 10-16 athletes.
...
### Medals
Let's do some work investigating just the medalists. One thing to note is that I choose to count every medal given out for each athlete for the team event instead of giving 1 medal to each country that placed. I feel this gives a better metric for how well a team is doing overall, since it gives the most weight to the most representative event. However, we will see that the performance in the team event is not the end-all-be-all of team performance as there are 12 other medals to be won on the individual events.
```{r medal processing}
...
The Soviet Union is clearly the leader in total medal count of 288, which is way ahead of the USA in second place with 177 medals. It also leads in the number of golds and silvers.
...
This is very interesting. It seems that China's medal performance might be related to the United States's medal performance. My theory for this is that China and USA have very comparable teams and their medal counts will respond similarly to external factors, such as other teams becoming stronger or weaker. There is a divergence in 2016 due to poor individual medal performance from China, even though they won team titles in both men's and women's. 

Now let's look at the individual medalists in terms of total medals and number of gold medals won.
```{r top medalists data}
top_totals <- medalists %>%
  group_by(Name, Team) %>%
  summarise(Total=n()) %>%
  arrange(desc(Total)) %>%
  head(10)
...
```{r top total medals plot}

top_totals$Name <- factor(top_totals$Name, levels=rev(top_totals$Name))
...
This plot should give an idea as to who was consistently great in multiple events. Additionally, the highest total medal winners tend to have survived multiple Olympic cycles, which is difficult to do because of how short a gymnast's competitive life-time usually is.

```{r top gold medals plot}
top_golds$Name <- factor(top_golds$Name, levels=rev(top_golds$Name))
...
Another way of looking at things is to see who won the most gold medals. Larsya Latynina is the clear leader in both total medal count and the number of gold medals for the women. On the men's side, there are three different candidates. Sawao Kato leads in gold medals, but both Borys Shakhlin and Nikolay Andrianov beat him in total medal count. My pick in the men's side would have to be Andrianov, since he has the same number of gold medals as Shakhlin, but leads in total medal count.

### Conclusion
This exploratory data analysis of gymnastics at the summer Olympics showed that there are some interesting aspects and trends in the athlete data that are worth investigating further. Here's a list of things that I want to pursue at a later time:

* The ages, heights, weights of medalists and if they differed from that year's competition.
* More detail about how changes in the sport over time changed the athletes.
* See how men's and women's teams perform differently in the same country.
* Predict the best teams for the 2020 games.
* Further investigate the strange China/USA medal relationship
* Visualize the gymnast of the decade for both men and women.